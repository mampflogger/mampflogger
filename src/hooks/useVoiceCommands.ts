...
const COMMANDS: VoiceCommand[] = [
  ...
  { patterns: [/\bgewicht(?!s?[\s-]?verlust|s?[\s-]?verlauf)\b/i, /\bwiegen\b/i, /\bwaage\b/i], action: "section:gewicht" },
  { patterns: [/\bbmi\b/i, /\bbody.?mass.?index\b/i], action: "section:bmi" },
  ...
  { patterns: [/\bgewichts.?verlauf\b/i, /\bgewichtsverlauf\b/i], action: "section:gewichts-verlauf" },
  ...
  { patterns: [/\bsicherung\b/i], action: "section:backup" },
  { patterns: [/\bbackup\b/i], action: "backup-create" },
  ...
  { patterns: [/^\s*storno\s*[.!?]?\s*$/i], action: "field:storno" },
...
const FUZZY_KEYWORD_MAP: [string, string][] = [
  ...
  ["gewichtsverlauf", "section:gewichts-verlauf"],
  ["bmi", "section:bmi"],
...
export const SECTION_PAGE_MAP: Record<string, "log" | "weekly"> = {
  ...
  "section-gewichts-verlauf": "weekly",
  ...
  const voice = useSpeechRecognition({
    onResult: useCallback((transcript: string, isInterim: boolean) => {
      ...
      if (!isInterim) {
        ...
        // 0b. If Tagesübersicht is actively focused/selected and transcript matches a table keyword,
