...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Du bist ein Ernährungs-Experte. Du berechnest Nährwerte für Zutatenlisten.

Du bekommst eine Liste von Zutaten mit Mengenangaben. Berechne die realistischen Nährwerte.
...
Antworte NUR mit einem JSON-Objekt:
{
  ...
  "ingredients": [
    { "name": "Hüttenkäse", "amount": "300g", "isMain": true }
  ]
}
...
- Gib die bereinigte Zutatenliste mit korrekten name/amount/isMain Feldern zurück`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Berechne die Nährwerte für folgende Zutatenliste:\n${ingredientList}` },
        ],
      }),
    });
